...
def train_conv_net(datasets,
    ...
    hidden_units = [x,y] x is the number of feature maps (per filter window),
    and y is the penultimate layer
    ...
    grad_updates = sgd_updates_adadelta(params, dropout_cost, lr_decay,
                                        1e-6, sqr_norm_lim)
    ...
    val_model = theano.function(
        [index],
        classifier.errors(y),
        givens={x: val_set_x[index * batch_size: (index + 1) * batch_size],
...
if __name__ == "__main__":
    ...
    for i in r:
        datasets = make_idx_data_cv(revs, word_idx_map, i,
                                    max_l=56, k=300, filter_h=5)
